...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "OffshoreCoin " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    // the prooffshore was closed and restarted.  Not an issue on windows.
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. OffshoreCoin is probably already running."), addrBind.ToString().c_str());
        else
